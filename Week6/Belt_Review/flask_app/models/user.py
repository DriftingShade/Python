# Replace all companies with the proper name as well as the DB name!
...
class User:
    DB = "companies_db"  # Database name goes here!
    ...
    @classmethod
    def find_by_id_with_companies(cls, user_id):
        query = """SELECT * FROM users LEFT JOIN companies ON users.id = 
        companies.user_id WHERE users.id = %(user_id)s"""
        data = {"user_id": user_id}
        ...
        for each_dict in list_of_dicts:
            if each_dict["companies.id"] != None:
                company_data = {
                    "id": each_dict["companies.id"],
                    "name": each_dict["name"],
                    "information": each_dict["information"],
                    "created_at": each_dict["companies.created_at"],
                    "updated_at": each_dict["companies.updated_at"],
                    "user_id": each_dict["user_id"],
                }
                company = company.Company(company_data)
